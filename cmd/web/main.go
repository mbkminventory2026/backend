...
func main() {
	...
	dataApproveCuttingPlanUseCase, err := usecase.NewDataApproveCuttingPlanUseCase(queries, dbPool)
	if err != nil {
		logger.Error("failed to initialize data approve cutting plan usecase", slog.String("error", err.Error()))
		dbPool.Close()
		os.Exit(1)
	}
	...
	materialListExcelImportUseCase, err := usecase.NewMaterialListExcelImportUseCase(queries, dbPool)
	if err != nil {
		logger.Error("failed to initialize material list excel import usecase", slog.String("error", err.Error()))
		dbPool.Close()
		os.Exit(1)
	}
	materialListExcelImportTemplateUseCase, err := usecase.NewMaterialListExcelImportTemplateUseCase(queries)
	if err != nil {
		logger.Error("failed to initialize material list excel import template usecase", slog.String("error", err.Error()))
		dbPool.Close()
		os.Exit(1)
	}
	...
	prInternalExcelExportUseCase, err := usecase.NewPRInternalExcelExportUseCase(excelRenderer, transactionDocumentUseCase)
	if err != nil {
		logger.Error("failed to initialize pr internal excel export usecase", slog.String("error", err.Error()))
		dbPool.Close()
		os.Exit(1)
	}
	...
	transactionDocumentHandler, err := httpdelivery.NewTransactionDocumentHandler(
		transactionDocumentUseCase,
		poInternalExcelExportUseCase,
		prInternalExcelExportUseCase,
	)
	if err != nil {
	...
	materialListHandler, err := httpdelivery.NewMaterialListHandler(materialListUseCase, materialListExcelExportUseCase, materialListExcelImportUseCase, materialListExcelImportTemplateUseCase)
	if err != nil {
	...
	dataApproveCuttingPlanHandler, err := httpdelivery.NewDataApproveCuttingPlanHandler(dataApproveCuttingPlanUseCase)
	if err != nil {
		logger.Error("failed to initialize data approve cutting plan handler", slog.String("error", err.Error()))
		dbPool.Close()
		os.Exit(1)
	}
