...
func (q *Queries) CreatePOClient(ctx context.Context, arg CreatePOClientParams) (CreatePOClientRow, error) {
...
type ListPaymentTermsRow struct {
	IDPaymentTerm int32  `json:"id_payment_term"`
	Kode          string `json:"kode"`
	Nama          string `json:"nama"`
}

func (q *Queries) ListPaymentTerms(ctx context.Context) ([]ListPaymentTermsRow, error) {
	rows, err := q.db.Query(ctx, listPaymentTerms)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []ListPaymentTermsRow
	for rows.Next() {
		var i ListPaymentTermsRow
		if err := rows.Scan(&i.IDPaymentTerm, &i.Kode, &i.Nama); err != nil {
			return nil, err
		}
...
func (q *Queries) UpdatePOClient(ctx context.Context, arg UpdatePOClientParams) (UpdatePOClientRow, error) {
